...
public class BST_AED2_UFP<Key extends Comparable<Key>, Value> {
    ...
    public static void main_old(String[] args) {
        BST_AED2_UFP<String, Integer> st = new BST_AED2_UFP<String, Integer>();
    ...
    public int keysCount(){
        return keysCount(root);
    }

    private int keysCount(Node x){
        if(x==null){
            return 0;
        }
        return keysCount(x.left) + keysCount(x.right) + 1;
    }


    /// Ficha 2 | Exercício 2 extra
    // ...

    public int evenKeysCount(){
        return evenKeysCount(root);
    }

    private int evenKeysCount(Node x){
        if(x == null){
            return 0;
        }
        if((Integer) x.key %2 == 0){
            return evenKeysCount(x.left) + evenKeysCount(x.right) +1;
        }else{
            return evenKeysCount(x.left) + evenKeysCount(x.right);
        }
    }
    ...
    public Iterable<Key> getKeysByLevel() {
        Queue<Key> queue = new Queue<Key>();
        for (Key x : this.levelOrder()) {
            queue.enqueue(x);
        }
        return queue;
    }
